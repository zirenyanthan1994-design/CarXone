...
export default function CustomerProfile() {
  ...
  const [newPickupDate, setNewPickupDate] = useState(""); // NEW
  const [newDropoffDate, setNewDropoffDate] = useState(""); // NEW
      ...
      <div className={`bg-white p-6 border rounded-xl flex flex-col gap-4 transition duration-300 shadow-sm ${isActive ? 'border-l-4 border-l-[#003366] border-gray-200 hover:shadow-md' : 'border-gray-200 hover:border-black opacity-95 hover:opacity-100'}`}>
        ...
        <div className="pt-4 flex justify-end gap-3 border-t border-gray-100 mt-2">
          ...
          <button 
            ...
            disabled={isChangePending || isCancelPending}
            className={`text-[10px] font-bold px-6 py-2.5 uppercase tracking-widest rounded transition ${isChangePending ? 'bg-orange-50 text-orange-400 cursor-not-allowed' : 'text-[#003366] bg-blue-50 hover:bg-[#003366] hover:text-white'}`}
          >
          ...
          <button 
            ...
            className={`text-[10px] font-bold px-6 py-2.5 uppercase tracking-widest rounded transition ${isCancelPending ? 'bg-red-50 text-red-300 cursor-not-allowed' : 'text-red-500 border border-red-100 bg-red-50 hover:bg-red-500 hover:text-white'}`}
          >
