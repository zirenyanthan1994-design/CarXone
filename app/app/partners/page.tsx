...
interface Booking {
  ...
  vendorRemark?: string; 
  paymentReceiptUrl?: string; // NEW: Added to view the screenshot
}
...
export default function PartnersDashboard() {
  ...
  const handleSaveSettings = async (e: React.FormEvent) => { e.preventDefault(); setIsSavingSettings(true); try { await setDoc(doc(db, "vendorSettings", "Dimapur Rentals"), { terms: termsText, upiId: upiId, commissionRate: Number(commissionRate), updatedAt: new Date().toISOString() }, { merge: true }); alert("Profile Settings successfully saved! Customers will now pay directly to this UPI ID."); } catch (error) { alert("Failed to save profile settings."); } finally { setIsSavingSettings(false); } };
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      alert("Success! The booking is now confirmed and the customer has been notified.");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-black font-sans flex flex-col relative">
      ...
      <header className="sticky top-0 z-40 bg-[#003366] border-b border-gray-800 shadow-sm text-white">
        <div className="flex items-center justify-between px-4 py-4 max-w-7xl mx-auto">
          ...
          <div className="flex items-center space-x-6 text-sm font-bold">
            <button onClick={() => setActiveTab("overview")} className={`transition ${activeTab === 'overview' ? 'text-blue-200' : 'hover:text-blue-300'}`}>Dashboard</button>
            <button onClick={() => setActiveTab("bookings")} className={`transition ${activeTab === 'bookings' ? 'text-blue-200' : 'hover:text-blue-300'} relative`}>
              Bookings & Requests
              {actionRequiredBookings.length > 0 && <span className="absolute -top-2 -right-3 bg-red-500 text-white text-[9px] px-1.5 py-0.5 rounded-full">{actionRequiredBookings.length}</span>}
            </button>
            <button onClick={() => setActiveTab("fleet")} className={`transition ${activeTab === 'fleet' ? 'text-blue-200' : 'hover:text-blue-300'}`}>My Fleet</button>
            <button onClick={() => setActiveTab("earnings")} className={`transition ${activeTab === 'earnings' ? 'text-blue-200' : 'hover:text-blue-300'}`}>Earnings</button>
            <button onClick={() => setActiveTab("terms")} className={`transition ${activeTab === 'terms' ? 'text-blue-200' : 'hover:text-blue-300'}`}>Profile & Settings</button>
            <button className="bg-white text-[#003366] px-5 py-1.5 rounded hover:bg-black hover:text-white transition">Log Out</button>
          </div>
        </div>
      </header>

      {/* --- MAIN DASHBOARD BODY --- */}
      <main className="flex-grow w-full max-w-7xl mx-auto px-4 py-8 flex flex-col gap-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div>
            <h2 className="text-2xl font-black text-black">Welcome back, Dimapur Rentals!</h2>
            <p className="text-gray-500 text-sm">Here is what is happening with your fleet today.</p>
          </div>
          <div className="flex items-center gap-4 mt-4 md:mt-0">
            {activeTab !== "overview" && (
              <button onClick={() => setActiveTab("overview")} className="text-xs font-bold bg-gray-100 text-gray-600 border border-gray-200 px-4 py-3 rounded hover:bg-gray-200 transition">
                ← Back to Overview
              </button>
            )}
            <a href="/partners/add-vehicle" className="bg-[#003366] text-white font-bold px-6 py-3 rounded hover:bg-black transition shadow-md flex items-center gap-2">
        ...
        {activeTab === "overview" && (
          <div className="text-center py-20 bg-white rounded-lg border border-gray-200">
            <h3 className="text-xl font-black text-[#003366] mb-2">Dashboard Overview</h3>
            <p className="text-gray-500 font-bold">Use the tabs above to manage your Fleet, Bookings, and Earnings.</p>
          </div>
        )}
            ...
            {actionRequiredBookings.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm border-2 border-red-200 overflow-hidden">
                <div className="bg-red-50 text-red-800 p-4 border-b border-red-200 flex justify-between items-center">
                  <h3 className="font-black text-lg flex items-center gap-2">⚠️ ACTION REQUIRED: CUSTOMER REQUESTS</h3>
                </div>
                <div className="p-6 flex flex-col gap-6">
                  {actionRequiredBookings.map(booking => (
                    <div key={booking.id} className="border border-gray-200 rounded-lg p-6 bg-gray-50 shadow-sm">
                      
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <h4 className="font-black text-xl text-black">{booking.vehicleName}</h4>
                          <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mt-1">Customer: {booking.customerName}</p>
                        </div>
                        <span className="font-black text-[#003366] text-2xl">₹{booking.totalPaid.toLocaleString()}</span>
                      </div>

                      {/* 1. If it's a NEW BOOKING (Pending Verification) */}
                      {booking.status === "pending_verification" && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-5 mb-4">
                          <p className="text-[10px] font-black text-[#003366] uppercase tracking-widest mb-2">New Booking Request</p>
                          <p className="text-sm font-bold text-black mb-4">The customer has uploaded their payment screenshot and is waiting for your confirmation.</p>
                          <button onClick={() => setVerifyingBooking(booking)} className="bg-[#003366] text-white text-xs font-black uppercase tracking-widest px-6 py-3 rounded hover:bg-black transition shadow-sm w-full md:w-auto">
                            Review & Verify Payment
                          </button>
                        </div>
                      )}

                      {/* 2. If it's a CANCELLATION Request */}
                      {booking.cancellationRequested && (
                        <div className="bg-red-100 border border-red-200 rounded-lg p-4 mb-4">
                          <p className="text-[10px] font-black text-red-800 uppercase tracking-widest mb-1">Cancellation Requested</p>
                          <p className="text-sm font-bold text-red-900">Reason: "{booking.cancellationReason}"</p>
                          
                          <div className="flex gap-3 mt-4">
                            <button onClick={() => handleAcceptCancel(booking)} className="bg-red-600 text-white text-xs font-black uppercase tracking-widest px-6 py-2.5 rounded hover:bg-red-700 transition shadow-sm">
                              Accept Cancellation
                            </button>
                            <button onClick={() => openRejectModal(booking, "cancel")} className="bg-white border border-red-300 text-red-600 text-xs font-black uppercase tracking-widest px-6 py-2.5 rounded hover:bg-red-50 transition">
                              Reject Request
                            </button>
                          </div>
                        </div>
                      ...
                      {booking.changeRequested && (
                        <div className="bg-blue-100 border border-blue-200 rounded-lg p-4 mb-4">
                          ...
                          <div className="flex gap-6 mb-2">
                            <div><span className="text-[10px] text-blue-600 font-bold uppercase block">New Pickup</span><span className="font-black text-black text-sm">{formatDate(booking.newPickupDate!)}</span></div>
                            <div><span className="text-[10px] text-blue-600 font-bold uppercase block">New Dropoff</span><span className="font-black text-black text-sm">{formatDate(booking.newDropoffDate!)}</span></div>
                          </div>
                          <p className="text-sm font-bold text-blue-900 mt-2">Message: "{booking.changeMessage}"</p>
                          
                          <div className="flex gap-3 mt-4">
                            <button onClick={() => handleAcceptChange(booking)} className="bg-[#003366] text-white text-xs font-black uppercase tracking-widest px-6 py-2.5 rounded hover:bg-black transition shadow-sm">
                              Accept New Dates
                            </button>
                            <button onClick={() => openRejectModal(booking, "change")} className="bg-white border border-blue-300 text-[#003366] text-xs font-black uppercase tracking-widest px-6 py-2.5 rounded hover:bg-blue-50 transition">
                              Reject Request
                            </button>
                          </div>
                        </div>
                    ...
                    {normalBookings.length === 0 ? (
                      ...
                      normalBookings.map((bk) => (
                        <tr key={bk.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                          ...
                          <td className="px-6 py-4 font-black text-[#003366]">₹{bk.totalPaid.toLocaleString()}</td>
                          <td className="px-6 py-4">
        ...
        {activeTab === "earnings" && (
          <div className="flex flex-col gap-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white rounded-xl border border-gray-200 p-6 flex flex-col justify-center items-center text-center shadow-sm">
                <span className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Gross Revenue (Paid by Customers)</span>
                <span className="text-4xl font-black text-gray-800">₹{totalGrossRevenue.toLocaleString()}</span>
              </div>
              <div className="bg-green-50 rounded-xl border border-green-200 p-6 flex flex-col justify-center items-center text-center shadow-sm relative overflow-hidden">
                <div className="absolute top-0 left-0 w-1 h-full bg-green-500"></div>
                <span className="text-xs font-bold text-green-800 uppercase tracking-widest mb-1">My Net Earnings (Vendor Share)</span>
                <span className="text-4xl font-black text-green-600">₹{totalVendorNet.toLocaleString()}</span>
              </div>
              <div className="bg-blue-50 rounded-xl border border-blue-200 p-6 flex flex-col justify-center items-center text-center shadow-sm relative overflow-hidden">
                <div className="absolute top-0 left-0 w-1 h-full bg-[#003366]"></div>
                <span className="text-xs font-bold text-[#003366] uppercase tracking-widest mb-1">Platform Share (Admin Cut)</span>
                <span className="text-4xl font-black text-[#003366]">₹{totalPlatformShare.toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="bg-gray-800 text-white p-4 flex justify-between items-center">
                ...
                <span className="bg-gray-700 text-xs font-bold px-3 py-1 rounded text-gray-300">
                  Current Commission Rate: {commissionRate}%
                </span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase text-[10px] font-black tracking-widest">
                    <tr>
                      <th className="px-6 py-4">Customer & Vehicle</th>
                      <th className="px-6 py-4">Gross Total Paid</th>
                      <th className="px-6 py-4 bg-orange-50 text-orange-800 border-l border-r border-orange-100">- Commission ({commissionRate}%)</th>
                      <th className="px-6 py-4 text-green-600 bg-green-50 font-black border-r border-green-100">Vendor Net</th>
                    </tr>
                  </thead>
                  <tbody>
                    {calculatedBookings.length === 0 ? (
                      ...
                      calculatedBookings.map((bk) => (
                        <tr key={bk.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                          ...
                          <td className="px-6 py-4 font-black text-gray-700">₹{bk.totalPaid.toLocaleString()}</td>
                          <td className="px-6 py-4 font-bold text-orange-600 bg-orange-50/50 border-r border-orange-50">- ₹{bk.commissionAmount.toLocaleString()}</td>
                          <td className="px-6 py-4 font-black text-green-600 text-base bg-green-50/50 border-r border-green-50">₹{bk.vendorKeeps.toLocaleString()}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
            ...
            <div className="p-8">
              ...
              <form onSubmit={handleSaveSettings} className="space-y-8 max-w-3xl">
                <div className="bg-blue-50 border border-blue-100 p-6 rounded-lg">
                  ...
                  <input type="text" required value={upiId} onChange={(e) => setUpiId(e.target.value)} placeholder="e.g. yourbusiness@sbi or 9876543210@paytm" className="w-full border-2 border-white rounded-lg p-4 focus:border-[#003366] outline-none text-sm font-bold text-black shadow-sm" />
                </div>

                <div className="bg-green-50 border border-green-200 p-6 rounded-lg shadow-sm">
                  <div className="flex items-center gap-3 mb-2">
                    <label className="block text-sm font-black text-green-800 uppercase tracking-widest">Platform Commission (%)</label>
                  </div>
                  <p className="text-xs text-green-700 mb-4 font-medium">Are you willing to give the platform a commission for bringing you confirmed bookings? Enter the percentage here. This is automatically deducted from your ledger.</p>
                  <div className="relative w-full md:w-1/3">
                    <input type="number" min="0" max="100" value={commissionRate} onChange={(e) => setCommissionRate(Number(e.target.value))} placeholder="e.g. 5" className="w-full border-2 border-white rounded-lg p-4 pl-12 focus:border-green-600 outline-none text-lg font-black text-green-800 shadow-sm" />
                    <span className="absolute left-4 top-1/2 transform -translate-y-1/2 font-black text-green-400 text-lg">%</span>
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Standard Rental Conditions</label>
                  <textarea rows={8} value={termsText} onChange={(e) => setTermsText(e.target.value)} placeholder="- Fuel must be returned at the same level." className="w-full border border-gray-300 rounded-lg p-4 focus:border-[#003366] outline-none bg-gray-50 focus:bg-white transition text-sm font-bold text-black" />
                </div>
                
                <div className="pt-6 border-t border-gray-100">
                  <button type="submit" disabled={isSavingSettings} className="bg-black text-white font-black text-sm uppercase tracking-widest px-8 py-4 rounded shadow hover:bg-[#003366] transition w-full md:w-auto">
        ...
        {activeTab === "fleet" && (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div className="bg-gray-800 text-white p-4 flex justify-between items-center">
              ...
              <button onClick={() => setDiscountTarget('all')} className="bg-green-500 hover:bg-green-400 text-white text-xs font-black px-4 py-2 rounded shadow transition flex items-center gap-2">
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase text-xs">
                  <tr>
                    ...
                    <th className="px-6 py-3 font-bold">Vehicle Info</th>
                    <th className="px-6 py-3 font-bold">Daily Rate</th>
                  ...
                  {myVehicles.map((vehicle) => {
                    ...
                    return (
                      <tr key={vehicle.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                        <td className="px-6 py-4">
                          <div className="w-16 h-12 bg-gray-200 rounded overflow-hidden shadow-sm relative">
                        ...
                        <td className="px-6 py-4 flex flex-col">
                          {vehicle.discount && vehicle.discount.value > 0 ? (
                            ...
                            <span className="font-bold text-[#003366] mt-2">₹{vehicle.basePrice}</span>
                          )}
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4 text-right flex justify-end gap-2 flex-wrap max-w-[320px] ml-auto">
                          
                          <button 
                            onClick={() => {
                              if (!isFeaturePending) setFeaturingVehicle(vehicle);
                            }} 
                            className={`text-xs font-bold px-3 py-1.5 rounded border transition ${isFeatured ? 'bg-yellow-100 text-yellow-800 border-yellow-300' : isFeaturePending ? 'bg-orange-100 text-orange-800 border-orange-300 cursor-not-allowed' : 'bg-yellow-50 text-yellow-700 border-yellow-200 hover:bg-yellow-400 hover:text-yellow-900'}`}
                            disabled={isFeaturePending}
                          >
                            {isFeatured ? '★ Featured' : isFeaturePending ? '⏳ Feature Pending' : '🌟 Feature'}
                          </button>
                          
                          <button onClick={() => setDiscountTarget(vehicle)} className="text-xs font-bold bg-green-50 text-green-700 px-3 py-1.5 rounded border border-green-200 hover:bg-green-600 hover:text-white transition">% Discount</button>
                          <button onClick={() => setBlockingVehicle(vehicle)} className="text-xs font-bold bg-orange-50 text-orange-700 px-3 py-1.5 rounded border border-orange-200 hover:bg-orange-100 transition">Block Dates</button>
                          <button onClick={() => setEditingVehicle(vehicle)} className="text-xs font-bold bg-gray-100 text-gray-700 px-3 py-1.5 rounded border border-gray-200 hover:bg-gray-200 transition">Full Edit</button>
                          <button onClick={() => handleDelete(vehicle.id)} className="text-xs font-bold bg-red-50 text-red-600 px-3 py-1.5 rounded border border-red-200 hover:bg-red-600 hover:text-white transition">Delete</button>
                        </td>
                      </tr>
                    );
                  })}
      ...
      {editingVehicle && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in duration-200">
            <div className="bg-[#003366] text-white p-5 flex justify-between items-center sticky top-0 z-10 shadow-md">
              <h3 className="font-black text-xl">Edit Vehicle: {editingVehicle.brand} {editingVehicle.model}</h3>
            ...
            <form onSubmit={handleSaveEdit} className="p-8 flex flex-col gap-8 bg-gray-50">
              <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm flex flex-col gap-4">
                <h4 className="text-lg font-black text-[#003366] border-b border-gray-100 pb-2">1. Basic Information</h4>
              ...
              <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm flex flex-col gap-4">
                <h4 className="text-lg font-black text-[#003366] border-b border-gray-100 pb-2">2. Pricing Strategy</h4>
                <div>
                  ...
                  <div className="bg-white p-4 border border-blue-200 rounded-lg shadow-sm mt-4">
                    ...
                    <div className="flex flex-col gap-3">
                      {(editingVehicle.destinations && editingVehicle.destinations.length > 0 ? editingVehicle.destinations : [{city: "", price: ""}]).map((dest, index) => (
                        <div key={index} className="flex items-end gap-3 bg-white p-3 rounded border border-blue-100">
                          ...
                          {((editingVehicle.destinations && editingVehicle.destinations.length > 0 ? editingVehicle.destinations : [{city: "", price: ""}]).length) > 1 && <button type="button" onClick={() => removeEditDest(index)} className="bg-red-100 text-red-600 p-2.5 rounded hover:bg-red-600 hover:text-white transition">X</button>}
                        </div>
                      ))}
                    ...
                    <div className="flex flex-col gap-3">
                      {(editingVehicle.kmTiers && editingVehicle.kmTiers.length > 0 ? editingVehicle.kmTiers : [{km: "", price: ""}]).map((tier, index) => (
                        <div key={index} className="flex items-end gap-3 bg-white p-3 rounded border border-blue-100">
                          ...
                          {((editingVehicle.kmTiers && editingVehicle.kmTiers.length > 0 ? editingVehicle.kmTiers : [{km: "", price: ""}]).length) > 1 && <button type="button" onClick={() => removeEditKmTier(index)} className="bg-red-100 text-red-600 p-2.5 rounded hover:bg-red-600 hover:text-white transition">X</button>}
                        </div>
                      ))}
              ...
              <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm flex flex-col gap-4">
                <h4 className="text-lg font-black text-[#003366] border-b border-gray-100 pb-2">3. Visibility & Status</h4>
                <div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="bg-orange-500 text-white p-4 flex justify-between items-center">
              <h3 className="font-black text-lg">Block Calendar: {blockingVehicle.model}</h3>
            ...
            <div className="p-6 flex flex-col gap-6">
              <form onSubmit={handleSaveBlockedDates} className="flex flex-col gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                <button type="submit" className="w-full bg-orange-500 text-white font-bold py-2 rounded hover:bg-orange-600 transition text-sm">+ Add to Blocked List</button>
              </form>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="bg-green-600 text-white p-4 flex justify-between items-center">
              <h3 className="font-black text-lg">{discountTarget === 'all' ? 'Apply Global Discount' : `Discount: ${discountTarget.model}`}</h3>
              <button onClick={() => setDiscountTarget(null)} className="text-white hover:text-black font-bold text-xl">&times;</button>
            </div>
            <form onSubmit={handleSaveDiscount} className="p-6 flex flex-col gap-6">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="bg-yellow-400 text-yellow-900 p-4 flex justify-between items-center">
              <h3 className="font-black text-lg">Feature Vehicle: {featuringVehicle.model}</h3>
            ...
            <div className="p-6 flex flex-col gap-6">
              ...
              <div>
                ...
                <select value={featureDays} onChange={(e) => setFeatureDays(Number(e.target.value))} className="w-full border-2 border-gray-200 rounded-lg p-4 font-black text-black outline-none focus:border-yellow-400 cursor-pointer">
              ...
              <div className="flex justify-between items-center bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                <span className="text-3xl font-black text-black">₹{featureDays * FEATURE_RATE_PER_DAY}</span>
              </div>
      ...
      {featuringVehicle && showAdminPaymentModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md flex flex-col items-center relative">
            ...
            <h2 className="text-2xl font-black text-black mb-2 text-center leading-tight">Pay Platform Admin</h2>
            <p className="text-gray-500 text-sm mb-6 text-center font-medium px-4">Pay securely via UPI, then upload a screenshot of your successful transaction.</p>
            <div className="text-5xl font-black text-black mb-8 border-b-2 border-gray-100 pb-8 w-full text-center">₹{featureDays * FEATURE_RATE_PER_DAY}</div>
            
            <a href={`upi://pay?pa=${ADMIN_UPI_ID}&pn=${encodeURIComponent(ADMIN_NAME)}&am=${featureDays * FEATURE_RATE_PER_DAY}&cu=INR`} className="w-full bg-[#003366] text-white font-black text-lg tracking-wide py-4 rounded-xl hover:bg-black transition shadow-lg flex items-center justify-center gap-3 mb-4">
            ...
            <div className="w-full bg-gray-50 border-2 border-gray-100 rounded-xl p-4 flex justify-between items-center mb-6">
              <div>
                ...
                <p className="font-bold text-black text-lg">{ADMIN_UPI_ID}</p>
              </div>
              <button onClick={handleCopyAdminUpi} className={`${copiedAdminUpi ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'} text-xs font-black uppercase tracking-widest px-4 py-2.5 rounded transition shadow-sm`}>
            ...
            <div className="w-full mb-6">
              ...
              <input type="file" accept="image/*" onChange={(e) => setFeatureScreenshot(e.target.files ? e.target.files[0] : null)} className="w-full text-sm font-bold border-2 border-blue-200 bg-blue-50 rounded-lg p-2 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-xs file:font-bold file:bg-[#003366] file:text-white hover:file:bg-black cursor-pointer transition" />
            </div>

            <div className="flex gap-4 w-full mt-2">
              <button onClick={() => { setShowAdminPaymentModal(false); setFeaturingVehicle(null); setFeatureScreenshot(null); }} className="w-1/3 bg-white border-2 border-gray-200 text-gray-500 font-bold py-3.5 rounded-xl hover:bg-gray-50 transition" disabled={isSubmittingFeature}>Cancel</button>
              <button onClick={handleFeatureSubmit} disabled={!featureScreenshot || isSubmittingFeature} className={`w-2/3 text-white font-black py-3.5 rounded-xl transition shadow-md flex items-center justify-center gap-2 ${!featureScreenshot || isSubmittingFeature ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600'}`}>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden relative">
            <div className="bg-[#003366] text-white p-5 flex justify-between items-center">
              <h3 className="font-black text-lg uppercase tracking-widest">Verify New Booking</h3>
              <button onClick={() => setVerifyingBooking(null)} className="text-gray-400 hover:text-white font-bold text-2xl">&times;</button>
            </div>
            ...
            <div className="p-8 flex flex-col gap-6">
              ...
              <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
                <div className="w-12 h-12 bg-[#003366] text-white rounded-full flex items-center justify-center text-xl font-black uppercase">
                ...
                <div>
                  ...
                  <p className="font-bold text-black">{verifyingBooking.customerName}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-black text-[#003366] text-lg">₹{verifyingBooking.totalPaid.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Pickup Date</p>
                  <p className="font-bold text-black text-sm">{formatDate(verifyingBooking.pickupDate)}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Drop-off Date</p>
                  <p className="font-bold text-black text-sm">{formatDate(verifyingBooking.dropoffDate)}</p>
                </div>
              </div>
              ...
              <div className="bg-blue-50 border border-blue-100 p-4 rounded-lg">
                ...
                {verifyingBooking.specialRemarks && (
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-700 italic">"{verifyingBooking.specialRemarks}"</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <a 
                  ...
                  className="w-full bg-gray-100 text-gray-600 border border-gray-200 font-bold text-sm uppercase tracking-widest py-3 rounded-lg hover:bg-gray-200 hover:text-black transition text-center flex items-center justify-center gap-2"
                >
              ...
              {/* Action Buttons */}
              <div className="flex gap-4 w-full mt-2 pt-4 border-t border-gray-100">
                <button 
                  onClick={() => openRejectModal(verifyingBooking, "booking")} 
                  className="w-1/3 bg-white border-2 border-red-200 text-red-500 hover:bg-red-50 hover:border-red-300 font-bold py-3.5 rounded-xl transition"
                ...
                <button 
                  ...
                  disabled={isSubmittingVerify}
                  className={`w-2/3 text-white font-black py-3.5 rounded-xl transition shadow-md ${isSubmittingVerify ? 'bg-green-400 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600'}`}
                >
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden relative">
            <div className="bg-gray-900 text-white p-5 flex justify-between items-center">
              <h3 className="font-black text-lg uppercase tracking-widest">Reject Request</h3>
              <button onClick={() => setShowRejectModal(false)} className="text-gray-400 hover:text-white font-bold text-2xl">&times;</button>
            </div>
            ...
            <div className="p-8 flex flex-col gap-6">
              ...
              <div>
                <label className="block text-xs font-black text-gray-500 uppercase tracking-widest mb-2">Reason for Rejection (Required) *</label>
                <textarea 
                  rows={4}
                  required
                  value={rejectReason}
                  ...
                  className="w-full border-2 border-gray-200 rounded-lg p-4 text-sm font-bold text-black focus:border-gray-900 outline-none transition bg-gray-50 focus:bg-white"
                />
              </div>
              ...
              <button 
                ...
                className={`w-full text-white font-black text-sm uppercase tracking-widest py-4 rounded-xl transition shadow-md ${!rejectReason || isSubmittingReject ? 'bg-gray-300 cursor-not-allowed' : 'bg-gray-900 hover:bg-black'}`}
              >
...
function StatCard({ title, value, subtitle, highlight = false, onClick, isActive = false }: { title: string, value: string, subtitle: string, highlight?: boolean, onClick?: () => void, isActive?: boolean }) {
  const baseStyle = "p-6 rounded-lg border shadow-sm flex flex-col cursor-pointer transition transform hover:-translate-y-1";
  ...
  return (
    <div className={`${baseStyle} ${colorStyle}`} onClick={onClick}>
      <span className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">{title}</span>
      <span className={`text-3xl font-black ${highlight ? 'text-red-600' : 'text-[#003366]'} mb-1`}>{value}</span>
